        ...
        <div class="results-container">
          ...
          <div class="results">
            ...
            <div class="song" v-for="song in songs">
              ...
              <div v-else class="dl"><a :href="`${host}/${song.songId}.m4a`" :download="acceptableName(song)"><svg height="25px" width="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12 3V16M12 16L16 11.625M12 16L8 11.625" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M15 21H9C6.17157 21 4.75736 21 3.87868 20.1213C3 19.2426 3 17.8284 3 15M21 15C21 17.8284 21 19.2426 20.1213 20.1213C19.8215 20.4211 19.4594 20.6186 19 20.7487" stroke="#1C274C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg></a></div>
            </div>
          </div>  
        </div>
      </div>
...
export default{
  ...
  async mounted(){
    //if (import.meta.env.VITE_DEV){
      ...
      const queue = await ((await fetch(QUEUE + "?" + new URLSearchParams({songname: this.searchbar}))).json())
      this.songs = queue
      console.log(queue)
    //}
  },
  ...
  methods:{
    ...
    async handleSearch(){
      ...
      setTimeout(async ()=>{
        if ( (Date.now() - this.lastLetter)>1000 &&  !this.clear){
          ...
          const res = await ((await fetch(GENIUS + "?" + new URLSearchParams({songname: this.searchbar}))).json())
    ...
    async add(){
      ...
      const res = await ((await fetch(ADD + "?" + new URLSearchParams({
        songname: this.searchbar, 
        songId   : songId
      }))).json())
      this.upnext    =  null
      this.clear     = true
